...
const {reviewInstructions,reviewSchema,reviewZ,stageExtractor,publishResearch}=await import('../src/publication.mjs');
...
try{
 ...
 const verdict=await respond('evaluation',[{id:'fixed-cases',role:'user',body:JSON.stringify({findings,sources,language:'es',coverage:[]})}], 'review-evaluation',[],
  {taskId:'eval',isolated:true,schema:reviewSchema,extract:stageExtractor(reviewZ),system:reviewInstructions});
 ...
 console.log(JSON.stringify({model:modelProfile('eval'),results},null,2));
 if(results.some(r=>!r.pass)||verdict.verdicts.length!==cases.length)throw new Error('Evidence review regression failed');
 // Also exercise the real isolated editor and final audit with a supported fact.
