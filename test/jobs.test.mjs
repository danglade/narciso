...
const completed={status:'completed',reply:'Resultado confirmado.',checkpoint:'Facts and evidence.',language:'es',notify:false,findings:[]};
const fakePublish=async(_db,_job,out)=>out.reply;
test('jobs snapshot the request, bind to owner, deduplicate, and wait for delivered acknowledgment',async()=>{
 ...
 let runs=0;const runner=createJobRunner(db,{autoStart:false,publish:fakePublish,run:async()=>{runs++;return completed;}});
...
test('foreground remains available while work runs; cancellation prevents late completion',async()=>{
 ...
 const runner=createJobRunner(db,{autoStart:false,publish:fakePublish,run:async()=>pending});const work=runner.step();
...
test('incomplete mail coverage cannot produce a completed job; checkpoint survives restart',async()=>{
 ...
 const runner=createJobRunner(db,{autoStart:false,publish:fakePublish,run:async()=>completed});await runner.step();
...
test('task result schema prevents unvalidated final messages; approval origins remain distinct',()=>{
 const legacy={status:completed.status,reply:completed.reply,checkpoint:completed.checkpoint};
 assert.deepEqual(extractTaskReply({type:'result',subtype:'success',structured_output:legacy}),legacy);
